...
if best_trials_nicon is not None and progressive_optim:
    trials_path = os.path.join(output_dir.replace("Figures","Results"), f"best_trials_CNN_{data_source}{adaptive_suffix}.json")
    try:
...
if best_trials_lgbm is not None and progressive_optim:
    trials_path = os.path.join(output_dir.replace("Figures","Results"), f"best_trials_LGBM_{data_source}{adaptive_suffix}.json")
    try:
        with open(trials_path, "w") as f:
...
if len(cnn_batch_sizes) > 0:
    trials_path = os.path.join(output_dir.replace("Figures","Results"), f"batch_sizes_CNN_{data_source}{adaptive_suffix}.json")
    try:
        with open(trials_path, "w") as f:
