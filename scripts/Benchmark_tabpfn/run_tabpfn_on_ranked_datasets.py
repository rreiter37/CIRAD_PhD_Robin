...
JSON_PATH = "Results/assoc_pp_model/All_datasets/Rank_datasets_difficulty/dataset_difficulty_ranking__ALL.json"
BASELINE_PATH = "scripts/Benchmark_tabpfn/pipeline_tabpfn.py"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Path to pipeline_tabpfn.py."
    )
...
def compute_n_pca(dataset_path, ratio=0.25, min_pca=10):
    """
    Compute number of PCA components as a fraction of calibration samples.
    """
    xcal_path = Path(dataset_path) / "Xcal.csv"
    if not xcal_path.exists():
        raise FileNotFoundError(f"Missing Xcal.csv in {dataset_path}")

    # Robust separator detection
    df = pd.read_csv(xcal_path, sep=None, engine="python")
    n_samples = df.shape[0]

    n_pca = max(min_pca, int(ratio * n_samples))
    return n_pca


def run_baseline(baseline_script, dataset_paths, workspace):
    """
    Call pipeline_tabpfn.py with the overridden dataset list.
    """
    for ds_path in dataset_paths:
        n_pca = compute_n_pca(ds_path, ratio=0.75)
