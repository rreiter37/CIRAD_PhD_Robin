...
def base_preproc_space() -> List[Tuple[str, List]]:
    """
    Stage-1 preprocessing space aligned with association_pp_model.py (simple_preprocs),
    excluding PCA (handled separately in the TabPFN search via the PCA spaces).

    IMPORTANT:
    - We keep exactly the same *single-step* preprocessings as in association_pp_model.py.
    - We do NOT generate 2-combinations here to keep Stage 1 fast and comparable.
    """
    return [
...
def std_preproc_space() -> List[Tuple[str, List]]:
    """
    Stage-1 standardization space.

    Since the Stage-1 preprocessing list is aligned with association_pp_model.py (and already includes
    several scaling/normalization operators such as SNV/Normalize/RNV/SimpleScale), we keep this
    space intentionally minimal to avoid duplicating effects.

    You can still introduce additional standardization strategies in Stage 2 if needed.
    """
    return [
        ("no_std", []),
    ]
...
def _stage1_eval_one(job: Dict[str, Any]) -> Optional[Tuple[SearchResult, Dict[str, Any], str]]:
    """
    Evaluate a single Stage-1 configuration.

    Returns:
        (SearchResult, candidate_dict, log_line) or None if failed.
...
def main():
    ...
    for ds_path in dataset_paths:
        ...
        # Build the full deterministic Stage-1 grid in a stable order.
