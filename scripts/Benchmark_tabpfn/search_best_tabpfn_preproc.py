...
def read_vector_csv(path: Path) -> np.ndarray:
    ...
    - auto-detect separator (',' vs ';') using pandas engine='python'
...
def base_preproc_space() -> List[Tuple[str, List]]:
    """
    TabPFN-friendly base preprocessings.
    Keep it small to avoid combinatorial explosion.
    """
    return [
        ("id", []),
...
def std_preproc_space() -> List[Tuple[str, List]]:
    """TabPFN-friendly standardizations."""
    return [
        ("no_std", []),
...
def steps_from_names(
    ...
    config_name = sanitize_name(f"{base_name}__{std_name}__{pca_name}__{rff_label}")
...
def main():
    ...
    for ds_path in dataset_paths:
            ...
            for (std_name, _) in std_space:
                for (pca_name, pca_obj) in pca1:
                    ...
                    pipeline = build_pipeline(
                        ...
                        use_split=True,
                        n_splits=args.stage1_n_splits,  # default 1
                        seed=args.seed,
                    ...
                    try:
                        ...
                        print(f"⚠️ Stage 1 failed for {dataset_name} / {config_name}: {e}")
                        continue

                    ds_key = get_single_dataset_key(preds_per_ds)
                    ds_pred = preds_per_ds[ds_key]["run_predictions"]  # Predictions object

                    val_rmse = get_top_rmse(ds_pred, partition="val")
                    if val_rmse is None:
                        print(f"⚠️ No val RMSE for {dataset_name} / {config_name}")
                        continue

                    # IMPORTANT: normalize by calibration range for stage 1/2 validation
                    val_nrmse = val_rmse / cal_range
        ...
        stage1_candidates = sorted(stage1_candidates, key=lambda x: x["val_nrmse"])
        topk = stage1_candidates[: args.stage1_top_k]

        print(f"\n[Stage 1] Top-{args.stage1_top_k} configs to refine:")
        for i, c in enumerate(topk, 1):
            print(f"  {i:>2}. {c['preprocessing']} | {c['standardization']} | {c['pca']} -> {c['val_nrmse']:.6f}")
        ...
        if args.stage2_method == "grid":
            ...
            rff_nc_list = sorted(set([nc_lo, int(np.clip((nc_lo + nc_hi) // 2, 32, 512)), nc_hi]))
            rff_sigma_list = [0.3, 1.0, 3.0]
            ...
            base_choices = [(c["preprocessing"], c["standardization"]) for c in topk]
            base_choice_labels = [f"{b}__{s}" for (b, s) in base_choices]
            ...
            def eval_config_on_folds(
                ...
                return float(val_rmse) / cal_range
            ...
            def objective(trial: optuna.Trial) -> float:
                ...
                idx = base_choice_labels.index(base_choice)
                base_name, std_name = base_choices[idx]

                pca_name = trial.suggest_categorical("pca", list(pca2_dict.keys()))
                use_rff = trial.suggest_categorical("use_rff", [False, True])

                if use_rff:
                    rff_nc = trial.suggest_int("rff_n_components", nc_lo, nc_hi)
                    rff_sigma = trial.suggest_float("rff_sigma", 0.1, 10.0, log=True)
            ...
            for cand in topk:
                ...
                std_name = cand["standardization"]
                pca_name = cand["pca"]
                base_choice = f"{base_name}__{std_name}"
            ...
            # Save Optuna per-dataset artifacts (these are small CSV/JSON, not nirs4all runs)
        ...
        final_test_nrmse = float(final_test_rmse) / test_range
